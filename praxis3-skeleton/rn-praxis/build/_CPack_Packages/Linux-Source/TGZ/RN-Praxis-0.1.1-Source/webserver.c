...
void send_stabilize() //generiert Stabilize
{
    ...
    fprintf(stderr, "Message sent in stabilize WORKING FROM: ID: %u, PORT: %u   TO: PORT: %u ID: %u \n\n", 
        self.id, self.port, successor.id, 
        successor.port);
    // Send the Stabilize message to the successor node
    dht_send(&stabilize_msg, &successor);
}

static void send_join(struct peer self_peer, const char* anchor_ip, const char* anchor_port){
    struct dht_message join_msg = create_msg(JOIN, 0, self_peer);
    struct peer anchor_peer = {
        .ip = { .s_addr = ntohl(inet_addr(anchor_ip)) },
        .port = strtoul(anchor_port, NULL, 10),
    };
    fprintf(stderr, "Message sent in join send TO: IP: %lu, PORT: %u with ID: %u \n", 
        (unsigned long)anchor_peer.ip.s_addr, 
        anchor_peer.port, self_peer.id);
    dht_send(&join_msg, &anchor_peer);
}

void* send_stabilize_wrapper(void* arg __attribute__((unused))) {
    while(true){
        while(time_ms() % 1000 != 0){};
        send_stabilize();
    }
    return NULL;
}
...
int main(int argc, char** argv) {
    ...
    if(no_stabilize == 0){
        if (pthread_create(&stabilize_thread, NULL, send_stabilize_wrapper, NULL) != 0) {
