...
void send_stabilize() //generiert Stabilize
{
    int stabilize_socket = socket(AF_INET, SOCK_DGRAM, 0);
    if (stabilize_socket == -1)
    {
        perror("UDP socket");
        exit(EXIT_FAILURE);
    }

    // Manually set up the sockaddr_in structure for the successor node
    struct sockaddr_in successor_addr;
    memset(&successor_addr, 0, sizeof(successor_addr));
    successor_addr.sin_family = AF_INET;
    successor_addr.sin_addr.s_addr = inet_addr(node_dht.SUCC_IP);
    successor_addr.sin_port = htons(node_dht.SUCC_PORT);
    // Create a Stabilize message
    chord_message stabilize_msg = create_chordMessage(2, (uint16_t)node_dht.SELF_ID, (uint16_t)node_dht.SELF_ID, htonl(inet_addr(node_dht.SELF_IP)), (uint16_t)node_dht.SELF_PORT);

    // Send the Stabilize message to the successor node
    int bytes_sent = sendto(stabilize_socket, &stabilize_msg, sizeof(chord_message), 0, (struct sockaddr *)&successor_addr, sizeof(successor_addr));
    if (bytes_sent == -1)
    {
        perror("sendto stabilize");
        close(stabilize_socket);
        exit(EXIT_FAILURE);
    }
...
void send_stabilize_update(int update_succ_node) 
{
    int stabilize_socket = socket(AF_INET, SOCK_DGRAM, 0);
    if (stabilize_socket == -1)
    {
    ...
    successor_addr.sin_family = AF_INET;
    successor_addr.sin_addr.s_addr = inet_addr(node_dht.SUCC_IP);
    successor_addr.sin_port = htons(update_succ_node);
    // Create a Stabilize message
    chord_message stabilize_msg = create_chordMessage(2, 0,(uint16_t)node_dht.SELF_ID , htonl(inet_addr(node_dht.SELF_IP)), (uint16_t)node_dht.SELF_PORT );
...
//funktion zum setzen des timers
void start_stabilize_timer()
{
    struct itimerspec timer_spec = {
        .it_value.tv_sec = 1,   //timer intervall in sekunden (ablaufzeit)
    };

    struct sigevent event = {
        .sigev_notify = SIGEV_THREAD,             // notify mit thread
        .sigev_notify_function = (void (*)(union sigval))send_stabilize, // thread funktion
        .sigev_value.sival_ptr = NULL              // argument für thread funktion
    };

    timer_t stabilize_timer;
    if (timer_create(CLOCK_REALTIME, &event, &stabilize_timer) == -1) //timer initialisieren
    {
        perror("timer_create");
        exit(EXIT_FAILURE);
    }

    if (timer_settime(stabilize_timer, 0, &timer_spec, NULL) == -1) //timer setzen
    {
        perror("timer_settime");
        exit(EXIT_FAILURE);
    }
}
void send_notify(int notify_node) //generiert notify
{ int notify_socket = socket(AF_INET, SOCK_DGRAM, 0);
    if (notify_socket == -1)
    {
    perror("UDP socket");
    exit(EXIT_FAILURE);
    }
    // Manually set up the sockaddr_in structure for the predecessor node
    struct sockaddr_in predecessor_addr;
    memset(&predecessor_addr, 0, sizeof(predecessor_addr));
    predecessor_addr.sin_family = AF_INET;
    predecessor_addr.sin_addr.s_addr = inet_addr(node_dht.PRED_IP);
    predecessor_addr.sin_port = htons(notify_node); //prev_Pred_port --> ohne Umgebungsvariable!!!
    // Create a Notify message
    chord_message notify_msg = create_chordMessage(3, 0, (uint16_t)node_dht.PRED_ID, htonl(inet_addr(node_dht.PRED_IP)), (uint16_t)node_dht.PRED_PORT);
    // Send the Notify message to the predecessor node
    int bytes_sent = sendto(notify_socket, &notify_msg, sizeof(chord_message), 0, (struct sockaddr *)&predecessor_addr, sizeof(predecessor_addr));
    if (bytes_sent == -1)
    {
...
void join_react(int join_port){
    ...
    memset(&predecessor_addr, 0, sizeof(predecessor_addr));
    predecessor_addr.sin_family = AF_INET;
    predecessor_addr.sin_addr.s_addr = inet_addr(node_dht.SELF_IP);
    predecessor_addr.sin_port = htons(join_port); //joining node

    chord_message notify_msg = create_chordMessage(3, 0, (uint16_t)node_dht.SELF_ID, htonl(inet_addr(node_dht.SELF_IP)), (uint16_t)node_dht.SELF_PORT);

    int bytes_sent = sendto(join_socket, &notify_msg, sizeof(chord_message), 0, (struct sockaddr *)&predecessor_addr, sizeof(predecessor_addr));
    if (bytes_sent == -1)
    {
    perror("sendto notify");
    close(join_socket);
...
int main(int argc, char **argv)
{
    ...
    if (argc == 4) // <-- Test Dht failed wegen dem Teil!! -> weil SUCC und PRED noch nicht gesetzt sind anfangs!, Nodes joinen nach einander!
    {
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < sizeof(sockets) / sizeof(sockets[0]); i++)
        {
            ...
            else if (s == udp_socket)
            {
                ...
                if (node_found == 0 && received_message->message_type == 0 )
                {
                ...
                else if ((node_found == 1 || node_found == 2) && received_message->message_type == 0 )
                {
                    ...
                    send_notify(htons(received_message->node_port));
                }else if(received_message-> message_type == 3){
                    send_stabilize_update( htons(received_message->node_port));
                }
                int node_found2 = found_node_responsible(htons(received_message->node_id));
                if(node_found2 == 0 && received_message -> message_type == 4){
                    int redirect_socket = socket(AF_INET, SOCK_DGRAM, 0);
                    if (redirect_socket == -1)
                    {
                        perror("UDP socket");
                    ...
                    nextAddr.sin_addr.s_addr = inet_addr(node_dht.SUCC_IP);
                    nextAddr.sin_port = htons(node_dht.SUCC_PORT);
                    chord_message *join_msg = (chord_message *)buf_udp;
                    int bytes = sendto(redirect_socket, join_msg, sizeof(chord_message), 0, (struct sockaddr *)&nextAddr, sizeof(nextAddr));
                    if (bytes == -1)
                    {
                        perror("sendto lookup");
                        close(redirect_socket);
                ...
                if ((node_found2 == 1 ))
                    ...
                    chord_message join_msg;
                    if (node_found2 == 1)
                    {
                        join_react(htons(received_message->node_port));
                    }
                    struct sockaddr_in requestAddr;
                    memset(&requestAddr, 0, sizeof(requestAddr));
                    requestAddr.sin_family = AF_INET;
                    requestAddr.sin_addr.s_addr = received_message->node_ip;
                    requestAddr.sin_port = received_message->node_port;
                    int bytes = sendto(dht_socket, &join_msg, sizeof(chord_message), 0, (struct sockaddr *)&requestAddr, sizeof(requestAddr));
                    if (bytes == -1)
                    {
                        perror("sendto lookup");
                        close(dht_socket);
